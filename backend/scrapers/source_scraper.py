    ...
    async def run_site(
        ...
        if await _is_shopify_store(base_url):
            ...
            if shopify_products is not None:
                ...
                if rate_limited:
                    # The fetch was TRUNCATED by Shopify rate limiting. The
                    # products we didn't see are still live on the store —
                    # archiving them would silently delete most of the catalog
                    # from the app. Skip archiving; keep previous data.
                    logger.warning(
                        "[SCRAPE] %s: fetch was truncated by rate limiting — "
                        "skipping archive of unseen products to protect existing data",
                        site_name,
                    )
                    stats["archived"] = 0
                    stats["partial"] = True
                    if self.scan_session_id:
                        with session_scope() as db:
                            ...
                            if sess:
                                sess.notes = (sess.notes or "") + (
                                    f"\n{site_name}: PARTIAL scan (rate limited) — no archiving performed"
                                )
                else:
