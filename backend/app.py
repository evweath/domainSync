...
async def on_startup():
    ...
    if sys.platform == "win32":
        # Windows-only: the ProactorEventLoop logs a scary
        # "Exception in callback _ProactorBasePipeTransport._call_connection_lost"
        # (ConnectionResetError / WinError 10054) every time a subprocess pipe
        # (e.g. the Playwright driver) closes. It is harmless noise — swallow
        # exactly that callback failure and let everything else report normally.
        loop = asyncio.get_running_loop()

        def _quiet_pipe_reset(loop, context):
            exc = context.get("exception")
            handle = context.get("handle")
            cb = getattr(handle, "_callback", None)
            cb_name = getattr(cb, "__qualname__", "") or ""
            if isinstance(exc, ConnectionResetError) and "_call_connection_lost" in cb_name:
                return
            loop.default_exception_handler(context)

        loop.set_exception_handler(_quiet_pipe_reset)
